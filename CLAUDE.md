...
- **The guide endpoint gzips its response whether or not the request asks for it**, so `lookup_to` fetches it with `curl --compressed`. Without that, every response is compressed bytes that fail the `<prog ` check — the retries run out and the recording never starts, while `tr` reports "Illegal byte sequence" on macOS. This was previously read as the endpoint "occasionally answering with binary."
...
- **The scheduler must be idempotent per minute**, since cron fires it every minute and the grace window spans many. `.rec_scheduler.state` keys `<id>|<entry index>|<execution time>` and is written *before* the recording starts; a failed entry is retried up to `MAX_ATTEMPTS` times, and a `running` entry left behind by a killed process unblocks after `STALE_RUNNING`.
- **Retries back off exponentially, and the grace window is what makes them possible.** Most failures are the guide endpoint rate-limiting us, so retrying on the next cron minute mostly failed three times in three minutes; the wait now starts at `RETRY_BACKOFF` (5 minutes) and doubles, putting the retries of a 3-attempt entry ~5 and ~15 minutes after it came due. A retry only happens while the entry is still inside `--grace`, so the backoff has to stay well under it — when it doesn't, the recorder logs that it is giving up instead of leaving the entry to fall out of the window unmentioned. Both give-up messages carry the recorder command line, since nothing will run it again.
